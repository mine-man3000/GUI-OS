...
void ProcessMousePacket(){
    if (!MousePacketReady) return;
        ...
        if (MousePosition.X > 319) MousePosition.X = 319;
...
void InitPS2Mouse(){
    ...
    port_byte_out(0x64, 0xA8); //enabling the auxiliary device - mouse

    MouseWait();
    port_byte_out(0x64, 0x20); //tells the keyboard controller that we want to send a command to the mouse
    MouseWaitInput();
    ...
    MouseWait();
    port_byte_out(0x60, status); // setting the correct bit is the "compaq" status byte
